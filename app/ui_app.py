...
# ---------- Util: downsample p/ MILP ----------
def _resample_inputs(price_df, gen_df, max_steps=9000, rule="1H"):
    """Se a série for muito longa, agrega para viabilizar MILP/PDF."""
    p = price_df.copy()
    g = gen_df.copy()
    p["datetime"] = pd.to_datetime(p["datetime"]); p = p.sort_values("datetime")
    g["datetime"] = pd.to_datetime(g["datetime"]); g = g.sort_values("datetime")
    T = len(g)
    if T <= max_steps:
        return p, g, None
    # Preços: média por hora; Geração (MWh passo): soma por hora
    p_r = p.set_index("datetime").resample(rule).mean(numeric_only=True).reset_index()
    g_r = g.set_index("datetime").resample(rule).sum(numeric_only=True).reset_index()
    note = f"Dados agregados automaticamente para **{rule}** (passos {T} → {len(g_r)}) para viabilizar solver/PDF."
    return p_r, g_r, note

# Tornar o pacote raiz visível
...
2. Preencha **parâmetros econômicos** e **limites de rede**.
3. Informe **CAPEX/eficiências** do BESS.
4. O app agrega automaticamente para **1h** quando a série é muito longa (para viabilizar solver/PDF).
5. Exporte **PDF** do cenário ou de todos os cenários.

**Abreviações:** LCOE, EBITDA, BESS, E_cap, P_cap, Throughput, CRF, P50/P90.
""")
...
if mode == "Cenário único":
    ...
    with c1:
        ...
        price_df_raw = pd.read_csv(price_file) if price_file is not None else None

    with c2:
        st.subheader("Geração da planta")
        gen_file = st.file_uploader("CSV (colunas: datetime, gen_MWh)", type=["csv"], key="gen")
        gen_df_raw = pd.read_csv(gen_file) if gen_file is not None else None

    if price_df_raw is None or gen_df_raw is None:
        st.info("Envie **preços** e **geração** para continuar.")
        st.stop()

    # Downsample se necessário (para MILP e PDF)
    price_df, gen_df, note = _resample_inputs(price_df_raw, gen_df_raw, max_steps=9000, rule="1H")
    if note:
        st.info(note)
    ...
    if not price_files or not gen_files or len(price_files) != len(gen_files) or len(labels) != len(price_files):
        st.info("Envie **mesmo número** de arquivos e rótulos.")
        st.stop()

    # Downsample cada par
    price_df_list, gen_df_list = [], []
    for pf, gf in zip(price_files, gen_files):
        p, g, _ = _resample_inputs(pd.read_csv(pf), pd.read_csv(gf), max_steps=9000, rule="1H")
        price_df_list.append(p); gen_df_list.append(g)
...
solver_time_limit_s = st.number_input("Limite de tempo do solver (s)", min_value=10, value=180, step=10)
...
# ---------------------------------------------------------
# Gráficos helpers (com amostragem p/ memória)
# ---------------------------------------------------------
...
def _make_compare_figure(baseline, with_bess):
    ...
    ax.set_xticks(list(x)); ax.set_xticklabels(items)
    ax.set_title("Comparação Sem BESS vs Com BESS"); ax.legend()
    return fig

def _make_schedule_figure(df, title="Schedule – energia (MWh por passo)"):
    dfp = _thin_df(df, max_points=2000)
    fig, ax = plt.subplots()
    ax.plot(dfp["datetime"], dfp.get("gen_MWh", 0), label="Geração")
    if "sold_from_gen_MWh" in dfp.columns:
        ax.plot(dfp["datetime"], dfp["sold_from_gen_MWh"], label="Venda direta")
    if "discharge_MWh" in dfp.columns:
        ax.plot(dfp["datetime"], dfp["discharge_MWh"], label="Descarga BESS")
    if "charge_from_grid_MWh" in dfp.columns:
        ax.plot(dfp["datetime"], dfp["charge_from_grid_MWh"], label="Carga da rede")
...
# ---------------------------------------------------------
# PDFs (format PT/BR + figuras “enxutas”)
# ---------------------------------------------------------
def _build_pdf_single(baseline, with_bess, fig_compare, fig_sched, project_name="Projeto", scenario_name="Cenário"):
    ...
    for line in lines:
        c.drawString(2.3*cm, y, line); y -= 0.5*cm

    y_img = y - 0.5*cm
    img_w = width - 4*cm; img_h = 7*cm
    if fig_compare:
        png1 = io.BytesIO(); fig_compare.savefig(png1, format="png", bbox_inches="tight"); png1.seek(0)
    ...
    if fig_sched:
        png2 = io.BytesIO(); fig_sched.savefig(png2, format="png", bbox_inches="tight"); png2.seek(0)
...
def _build_pdf_multi(results, compare_table, project_name="Energy+BESS – Batch"):
    ...
    c.line(2*cm, height-3.8*cm, width-2*cm, height-3.8*cm); c.showPage()
    ...
    for i, h in enumerate(headers[:4]): c.drawString(col_x[i], y, h)
    y -= 0.5*cm
    for i, h in enumerate(headers[4:], start=4): c.drawString(col_x[i-4], y, h)
    y -= 0.4*cm; c.line(2*cm, y, width-2*cm, y); y -= 0.35*cm; c.setFont("Helvetica", 9)

    for _, row in compare_table.iterrows():
        if y < 2.5*cm:
            c.showPage(); c.setFont("Helvetica-Bold", 13)
            c.drawString(2*cm, height-2.5*cm, "Resumo comparativo (continuação)")
            y = height - 3.3*cm; c.setFont("Helvetica", 9)
        ...
        y -= 0.45*cm
        c.drawRightString(col_x[0]+11.2*cm, y, fmt_pt(row["LCOE_base"], 2) if pd.notna(row["LCOE_base"]) else "-")
        c.drawRightString(col_x[1]+11.2*cm, y, fmt_pt(row["LCOE_com_BESS"], 2) if pd.notna(row["LCOE_com_BESS"]) else "-")
        c.drawRightString(col_x[2]+11.2*cm, y, euro(row["Receita_base_EUR_ano"], 0) if pd.notna(row["Receita_base_EUR_ano"]) else "-")
        c.drawRightString(col_x[3]+11.2*cm, y, euro(row["Receita_BESS_EUR_ano"], 0) if pd.notna(row["Receita_BESS_EUR_ano"]) else "-")
        c.drawRightString(col_x[0]+19.2*cm, y, fmt_pt(row["Throughput_MWh_ano"], 0) if pd.notna(row["Throughput_MWh_ano"]) else "-")
        y -= 0.55*cm
...
if mode == "Cenário único":
    ...
    # Exportar PDF (sempre funciona; se BESS falhou, gera só baseline)
    ...
    pdf_bytes = _build_pdf_single(
        ...
        fig_compare=(None if not res_bess.get("status_text", "").startswith("Solução") else _make_compare_figure(baseline, res_bess)),
        fig_sched=(None if not res_bess.get("status_text", "").startswith("Solução") else _make_schedule_figure(res_bess["schedule"], "Schedule – Com BESS")),
        project_name="Energy+BESS",
        scenario_name="Cenário único",
    )
    st.download_button("⬇️ Baixar PDF (narrativa + gráficos)", data=pdf_bytes,
                       file_name="energy_bess_report.pdf", mime="application/pdf")
...
# ---------------------------------------------------------
    ...
    if len(compare_table) > 0:
        ...
        ax.set_title("LCOE – Base vs Com BESS (por cenário)"); ax.legend()
        st.pyplot(fig); plt.close(fig)
    ...
    st.download_button("⬇️ Baixar PDF (todos os cenários)", data=pdf_all,
                       file_name="energy_bess_all_scenarios.pdf", mime="application/pdf")
