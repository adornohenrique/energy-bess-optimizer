...
def bess_capex_eur(E_cap_MWh, P_cap_MW, c_E_capex_eur_per_kwh, c_P_capex_eur_per_kw):
    """
    ...
    Converte internamente kWh->MWh e kW->MW.
    """
    if E_cap_MWh < 0 or P_cap_MW < 0:
        raise ValueError("E_cap_MWh e P_cap_MW devem ser >= 0")

    c_E_per_MWh = c_E_capex_eur_per_kwh * 1000.0  # EUR/MWh
    c_P_per_MW = c_P_capex_eur_per_kw * 1000.0    # EUR/MW

    return c_E_per_MWh * E_cap_MWh + c_P_per_MW * P_cap_MW


def annualized_bess_cost_eur(bess_capex_eur_value, rate_percent, n_years):
    """
...
def ebitda_annual_eur(
    ...
    bess_annual_cost_eur,
    extra_opex_eur=0.0,
):
    """
    EBITDA contábil simplificado:
...
def roi_from_ebitda(ebitda_annual_eur_value, total_capex_eur):
    """
    ROI contábil anual:
    ...
    if total_capex_eur <= 0:
        return 0.0
    return ebitda_annual_eur_value / total_capex_eur
